...
class BaseModel(pydantic.BaseModel):
    ...
    def factory(cls):
        ...
        class Factory(ModelFactory[cls]):
            ...
            @classmethod
            def get_provider_map(cls) -> dict[typing.Type, Any]:
                providers_map = super().get_provider_map()
                providers_map[EncryptedSecretBytes] = lambda: EncryptedSecretBytes(
                    "".join(
                        random.choices(
                            string.ascii_letters + string.digits,
                            k=random.randint(
                                EncryptedSecretBytes.min_length,
                                EncryptedSecretBytes.max_length,
                            ),
                        ),
                    ).encode(),
                )
                providers_map[NotEmptySecretStr] = lambda: NotEmptySecretStr(
                    "".join(
                        random.choices(
                            string.ascii_letters + string.digits,
                ...
                providers_map[NotEmptyStr] = lambda: NotEmptyStr(
                    "".join(
                        random.choices(
                            string.ascii_letters + string.digits,
                            k=random.randint(
                                NotEmptyStr.min_length or 1,
                                NotEmptyStr.max_length or 20,
                            ),
                        ),
                    ),
                )
